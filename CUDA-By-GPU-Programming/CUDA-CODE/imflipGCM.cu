...
// Improved kernel that copies an image from one part of the GPU memory 
// (ImgSrc) to another (ImgDst) {which must both be passed as integer pointers}
// This kernel does not use shared memory, only 32-bit pointers ...
...
{
    char            Flip = 'H';
    float           totalTime, tfrCPUtoGPU, tfrGPUtoCPU, kernelExecutionTime; // GPU code run times with tfr(transfer)
